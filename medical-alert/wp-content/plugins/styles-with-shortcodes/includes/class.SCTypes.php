...
<?php
	...
	function wp_uploader($field){
		$field->id = sprintf( "mce-wp_uploader-%s", $this->uid++ );
		$button_label = property_exists($field,'button_label') && !empty($field->button_label)?$field->button_label:__('Continue','sws');
?>
		<div class="fieldset">
			<?php if(!empty($field->description)) :?>
			<div class="description"><?php echo $field->description?></div>
			<?php endif;?>
			<label class="css-mce-label"><?php echo $field->label; ?></label>
			<div class="css-mce-input wp-media-buttons">
				<?php echo sprintf("<input id=\"%s\" name=\"sws_%s\" type=\"text\" class=\"mce-item sws_wp_uploader_inp %s\" value=\"%s\" rel=\"%s\" />",$field->id,$field->name,$this->get_classes($field),$field->default,$this->get_jsfunc($field)); ?>
				<a href="#" id="<?php echo $field->id?>-media-btn" data-target_field_id="#<?php echo $field->id?>" data-modal_title="<?php echo $field->label?>" data-modal_button="<?php echo $button_label?>" class="button sws_wp_uploader add_media" title="<?php _e('Add Media','sws') ?>">
					<span class="wp-media-buttons-icon"></span> <?php _e('Add Media','sws') ?></a>
