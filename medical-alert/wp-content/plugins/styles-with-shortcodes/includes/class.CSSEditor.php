...
<?php	
	...
	function media_buttons_context($context){
        global $wp_version;
		if($wp_version<3.5){
			$out = '<a id="sws-insert-tool-trigger-'.($this->instance++).'" class="sws-insert-tool-trigger" href="javascript:void(0);" title="'. __("Add Styles with Shortcodes", 'wpcss').'"><img src="'.WPCSS_URL."css/images/icon_shortcodes.png".'" alt="'. __("Add Styles with Shortcodes", 'wpcss') . '" /></a>';
		}else{
			$out = '<a id="sws-insert-tool-trigger-'.($this->instance++).'" class="button sws-insert-tool-trigger" href="javascript:void(0);" title="'. __("Add Styles with Shortcodes", 'wpcss').'"><span class="sws-icon-add-shortcode"></span>'.__('Add shortcode','wpcss').'</a>';
		}
		return $context . $out;
	}
