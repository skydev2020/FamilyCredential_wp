...
function set_wp_uploader(){
	jQuery(document).ready(function($){
		$('.sws_wp_uploader').unbind('click').on('click',function(e){
		    ...
		    var _modal = wp.media.frames.sws = wp.media({
		    ...
		    _modal.on( 'select', function() {
				...
				$(_target_id_field).val( attachment.url ).trigger('change');
		    });
		    _modal.open();
